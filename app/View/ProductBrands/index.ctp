...
<div class="content-body">
    <div class="row">
        <div class="col-xs-8 col-md-10">
            <?php
                echo $this->Form->create('ProductBrand', array('class'=>'form-inline'));
        ...
        <div class="col-xs-4 col-md-2 text-right">
            <a href="/ProductBrands/add"><i class="fa fa-plus-circle fa-fw fa-3x" data-toggle="tooltip" data-placement="left" title="Create Brand"></i></a>
        </div>
    </div>
    <div class="row">
        <div class="col-xs-12">
            <table class="table table-striped table-condensed table-data-list">
                ...
                <?php foreach ($productBrands as $k => $brand): ?>
                <tr data-toggle="modal" data-target="#recordAction" data-name="<?php echo $brand['ProductBrand']['name']; ?>" data-id="<?php echo $brand['ProductBrand']['id']; ?>">
                    <td class="text-center"><?php echo h($k+1); ?></td>
                    <td><?php echo h($brand['ProductBrand']['name']); ?>&nbsp;</td>
                    <td class="hidden-sm hidden-xs"><?php echo h($brand['ProductBrand']['name_en']); ?>&nbsp;</td>
                    <td class="hidden-sm hidden-xs"><?php echo h($brand['ProductBrand']['enable']); ?>&nbsp;</td>
                    <td class="hidden-sm hidden-xs fit-content"><?php echo h($brand['ProductBrand']['modified']); ?>&nbsp;</td>
                    <td class="hidden-sm hidden-xs text-center" style="width: 180px">
                        <div class="btn-group" role="group" aria-label="...">
                        <?php
                            echo $this->Html->link(__('View'), array('action' => 'view', $brand['ProductBrand']['id']), array('class'=>'btn btn-default btn-action'));
                            echo $this->Html->link(__('Edit'), array('action' => 'edit', $brand['ProductBrand']['id']), array('class'=>'btn btn-default btn-action'));
                            echo $this->Html->link(__('Delete'), array('action' => 'delete', $brand['ProductBrand']['id']), array('confirm' => __('Are you sure you want to delete # %s?', $brand['ProductBrand']['name']), 'class'=>'btn btn-default btn-action'));
                        ?>
                        </div>
                    </td>
                </tr>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body text-center">
                <?php
                    ...
                    echo $this->Html->link(__('Delete'), '#', array('id'=>'btnDel', 'confirm' => __('Are you sure you want to delete # %s?', $brand['ProductBrand']['name']), 'class'=>'btn btn-danger btn-lg btn-block icon-bin-before', 'style'=>'margin-top:5px'));
                ?>
            </div>
        </div>
    </div>
...
$('#recordAction').on('show.bs.modal', function (event) {
  ...
  modal.find('#btnView').attr('href','/ProductBrands/view/'+id);
  modal.find('#btnEdit').attr('href','/ProductBrands/edit/'+id);
  modal.find('#btnDel').attr('href','/ProductBrands/delete/'+id);
});
</script>